defmodule PokerHands.Rankers.FullHouseRanker do
  ...
  def tie(hand_black, hand_white) do
    ...
    black_card_values = List.flatten(Enum.map(hand_black_suits_3, fn(x) -> CardHelper.get_card_values(x) end))
    white_card_values = List.flatten(Enum.map(hand_white_suits_3, fn(x) -> CardHelper.get_card_values(x) end))
    ...
    if (RankerHelper.hands_are_equal(black_card_values_sorted, white_card_values_sorted)) do
      ...
      RankerHelper.compare(black_card_values_sorted, white_card_values_sorted)
    end
  end
end
